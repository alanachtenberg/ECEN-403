...
#include <inc/hw_types.h> //Defines common types and macros such as tBoolean andHWREG(x).
...
includes API functions such as SysCtlClockSet and SysCtlClockGet.*/
...
int main(void){
	...
	SysCtlClockSet(SYSCTL_SYSDIV_4|SYSCTL_USE_PLL|SYSCTL_XTAL_16MHZ|SYSCTL_OSC_MAIN);//using main osc and pll, sysdiv is 4, and external cystal is 16 mhz
