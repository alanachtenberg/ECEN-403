...
SysCtlClockSet(SYSCTL_SYSDIV_5|SYSCTL_USE_PLL|SYSCTL_XTAL_16MHZ|SYSCTL_OSC_MAIN); //Set clock speed = 40MHz
...
SysCtlPeripheralEnable(SYSCTL_PERIPH_GPIOB); // Enable the GPIO port containing the T0 CCP0 and CCP1 pins
SysCtlPeripheralEnable(SYSCTL_PERIPH_GPIOF); // Enable port F, containing T1CCP0, CCP1
...
// memmap.h - Defines base addresses for memory and peripherals (timers and gpio's)
// types.h - Defines common types (tboolean) and macros for hardware access
// ints.h - Defines interrupt assignments for PWM, timers, and gpio's
// sysctl.h - Defines function prototypes to control the peripherals
// interrupt.h - Defines function prototypes for NVIC interrupt controller
// pin_map.h - Maps peripherals to their respective pins
// gpio.h - Defines function prototypes and input arguments for GPIO API's
// timer.h - Defines function prototypes and input values for Timer API's
